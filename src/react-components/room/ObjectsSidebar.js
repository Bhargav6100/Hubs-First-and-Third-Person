...
export function NoObjects({ canAddObjects }) {
  return (
    <li className={styles.noObjects}>
      <p>
        <FormattedMessage id="objects-sidebar.no-objects" defaultMessage="There are no items in the Cart." />
      </p>
      {canAddObjects && (
        <p>
          <FormattedMessage id="objects-sidebar.place-menu-tip" defaultMessage="Use the place menu to add items." />
        </p>
      )}
    </li>
  );
    ...
    <Sidebar
      title={
        <FormattedMessage
          ...
          defaultMessage="Cart ({objectCount})"
          values={{ objectCount }}
        />
      }
